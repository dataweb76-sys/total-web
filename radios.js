...
const RADIO_SERVER = 'https://demographic-sugar-divided-cir.trycloudflare.com';
...
const _WMO = {
  ...
  45:'🌫️ Neblina',51:'🌦️ Llovizna leve',61:'🌧️ Lluvia leve',63:'🌧️ Lluvia',
  65:'🌧️ Lluvia intensa',80:'🌦️ Chaparrón',95:'⛈️ Tormenta',
};
(function initClima() {
  const el = document.getElementById('twClima');
  if (!el) return;
  async function fetchClima(lat, lon, ciudad) {
    try {
      const r = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weathercode,windspeed_10m,relativehumidity_2m&timezone=auto&wind_speed_unit=kmh`);
      const d = await r.json(); const c = d.current;
      el.innerHTML = `
        <div class="tw-temp">${Math.round(c.temperature_2m)}°C</div>
        <div class="tw-desc">${_WMO[c.weathercode]||'🌡️'}</div>
        <div class="tw-meta">💧 ${c.relativehumidity_2m}%&nbsp; 💨 ${Math.round(c.windspeed_10m)} km/h</div>
        <div class="tw-meta" style="margin-top:3px">📍 ${ciudad}</div>`;
    } catch(e) { el.innerHTML='<span class="tw-meta">Sin datos</span>'; }
  }
  if (!navigator.geolocation) { fetchClima(-36.6167,-64.2833,'Santa Rosa, LP'); return; }
  navigator.geolocation.getCurrentPosition(
    pos => {
      fetch(`https://nominatim.openstreetmap.org/reverse?lat=${pos.coords.latitude}&lon=${pos.coords.longitude}&format=json`)
        .then(r=>r.json())
        .then(g=>{ const c=g.address?.city||g.address?.town||g.address?.village||'tu ciudad'; fetchClima(pos.coords.latitude,pos.coords.longitude,c); })
        .catch(()=>fetchClima(pos.coords.latitude,pos.coords.longitude,'tu ubicación'));
    },
    ()=>fetchClima(-36.6167,-64.2833,'Santa Rosa, LP'),
    {timeout:6000}
  );
})();
...
(async function loadDolar() {
  ...
  try {
    ...
    if (!d.dolares) return;
    const mostrar = ['blue','oficial','tarjeta'];
    el.innerHTML = d.dolares.filter(x=>mostrar.includes(x.casa)).map(x=>`
      <div class="tw-drow">
        <span>${x.nombre}</span>
        <span><span class="tw-dcompra">C $${x.compra}</span>&nbsp;<span class="tw-dventa">V $${x.venta}</span></span>
      </div>`).join('');
    if (upd && d.updated) upd.textContent = new Date(d.updated).toLocaleTimeString('es-AR');
  } catch(e) { if(el) el.innerHTML='<span class="tw-meta">Sin datos</span>'; }
})();
...
function twFilter(btn, cat) {
  ...
  twRender(cat ? _twNoticias.filter(n=>n.categoria===cat) : _twNoticias);
}

function twRender(lista) {
  const el = document.getElementById('twNewsList');
  if (!el) return;
  if (!lista.length) { el.innerHTML='<span class="tw-meta">Sin noticias en esta categoría.</span>'; return; }
  el.innerHTML = lista.map(n=>`
    <a href="${n.url||'#'}" target="_blank" rel="noopener" class="tw-ni cat-${n.categoria||'local'}">
      <div class="tw-ni-meta">${n.categoria||''} · ${n.fuente||''} · ${n.fecha||''}</div>
      <div class="tw-ni-title">${n.titulo}</div>
      <div class="tw-ni-desc">${n.resumen}</div>
    </a>`).join('');
}
...
(async function loadNoticias() {
  try {
    ...
    const d = await r.json();
    if (!d.noticias) return;
    _twNoticias = d.noticias;
    twRender(_twNoticias);
    const upd = document.getElementById('twNewsUpd');
    if (upd && d.updated) upd.textContent = new Date(d.updated).toLocaleTimeString('es-AR');
  } catch(e){}
  setTimeout(loadNoticias, 15*60*1000);
})();
