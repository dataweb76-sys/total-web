...
const RADIO_SERVER = 'https://weekends-historic-brook-salt.trycloudflare.com';
...
function connectSocket() {
  ...
  // Noticias en tiempo real desde el admin
  socket.on('news_update', noticias => renderNoticias(noticias));
}

// ── Clima (open-meteo, La Pampa) ─────────────────────────────────────────
const WMO_DESC = {0:'Despejado',1:'Mayormente despejado',2:'Parcialmente nublado',3:'Nublado',45:'Niebla',51:'Llovizna',53:'Llovizna',55:'Llovizna fuerte',61:'Lluvia leve',63:'Lluvia',65:'Lluvia fuerte',71:'Nieve leve',80:'Chaparrones',95:'Tormenta'};
const WMO_ICON = {0:'☀️',1:'🌤️',2:'⛅',3:'☁️',45:'🌫️',51:'🌦️',53:'🌦️',55:'🌧️',61:'🌧️',63:'🌧️',65:'🌧️',71:'🌨️',80:'🌦️',95:'⛈️'};

async function loadClima() {
  const el = document.getElementById('climaContent');
  if (!el) return;
  try {
    const r = await fetch('https://api.open-meteo.com/v1/forecast?latitude=-36.62&longitude=-64.29&current=temperature_2m,relative_humidity_2m,wind_speed_10m,weathercode&timezone=America%2FArgentina%2FBuenos_Aires');
    const d = await r.json();
    const c = d.current;
    const code = c.weathercode;
    el.innerHTML = `
      <div class="clima-main">
        <div class="clima-icon">${WMO_ICON[code] || '🌡️'}</div>
        <div>
          <div class="clima-temp">${Math.round(c.temperature_2m)}°C</div>
          <div class="clima-desc">${WMO_DESC[code] || 'Variable'}</div>
        </div>
      </div>
      <div class="clima-extra">
        <div class="clima-pill">💧 <strong>${c.relative_humidity_2m}%</strong></div>
        <div class="clima-pill">💨 <strong>${Math.round(c.wind_speed_10m)} km/h</strong></div>
      </div>`;
  } catch(e) {
    if (el) el.innerHTML = '<div class="ep-empty">Sin datos</div>';
  }
}

// ── Dólar (desde servidor radio) ─────────────────────────────────────────
async function loadDolar() {
  const el = document.getElementById('dolarContent');
  if (!el) return;
  try {
    const r = await fetch(RADIO_SERVER + '/api/dolar');
    const d = await r.json();
    const filas = (d.dolares || [])
      .filter(x => ['Oficial','Blue','Bolsa','Tarjeta'].includes(x.nombre))
      .map(t => `
        <div class="dolar-row">
          <span class="dolar-nombre">${esc(t.nombre)}</span>
          <div class="dolar-vals">
            <span class="dolar-compra">C $${(t.compra||0).toLocaleString('es-AR')}</span>
            <span class="dolar-venta">V $${(t.venta||0).toLocaleString('es-AR')}</span>
          </div>
        </div>`).join('');
    el.innerHTML = filas || '<div class="ep-empty">Sin datos</div>';
  } catch(e) {
    if (el) el.innerHTML = '<div class="ep-empty">Sin conexión al servidor</div>';
  }
}

// ── Noticias (desde servidor radio) ──────────────────────────────────────
async function loadNoticias() {
  try {
    const r = await fetch(RADIO_SERVER + '/api/news');
    const d = await r.json();
...
function renderNoticias(noticias) {
  ...
  if (!noticias || noticias.length === 0) {
    el.innerHTML = '<div class="ep-empty">No hay noticias cargadas.</div>';
    return;
  }
  el.innerHTML = noticias.slice(0, 8).map(n => `
    <a href="${esc(n.url || '#')}" target="_blank" rel="noopener" class="noticia-item">
      <div class="noticia-titulo">${esc(n.titulo || '')}</div>
      ${n.resumen ? `<div class="noticia-resumen">${esc(n.resumen.slice(0,120))}${n.resumen.length > 120 ? '…' : ''}</div>` : ''}
      <div class="noticia-fuente">${esc(n.fuente || '')}</div>
    </a>`).join('');
}

// Cargar widgets al iniciar
loadClima();
loadDolar();
loadNoticias();
// Refrescar dólar y clima cada 30 min
setInterval(loadDolar,  30 * 60 * 1000);
